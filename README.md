> **온디바이스 AI 기반 주방 화재/연기 실시간 감지 및 Hands-Free 레시피 제어 시스템**

![Python](https://img.shields.io/badge/Python-3.9-3776AB?style=flat&logo=python&logoColor=white)
![YOLOv8](https://img.shields.io/badge/YOLOv8-Ultralytics-000000?style=flat)
![OpenCV](https://img.shields.io/badge/OpenCV-4.x-5C3EE8?style=flat&logo=opencv&logoColor=white)
![Platform](https://img.shields.io/badge/Platform-Windows-0078D6?style=flat&logo=windows&logoColor=white)

---

## 📌 1. 프로젝트 개요 (Overview)

요리 중 발생할 수 있는 **주방 화재 위험(연기)**을 실시간으로 감지하여 대형 사고를 예방하고, 식재료나 양념이 손에 묻어 디바이스를 직접 터치하기 힘든 상황에서 **비접촉 손동작(Gesture)**만으로 레시피/유튜브 화면 및 타이머를 제어하는 **온디바이스(On-Device) AI 소프트웨어**입니다.

### 💡 기획 배경 및 페인 포인트 (Pain Point)
1. **주방 안전 문제**: 요리 중 잠깐 자리를 비우거나 부주의로 인해 음식이 타면서 연기가 발생하고 화재로 이어지는 위험 존재.
2. **위생 및 기기 오염**: 양손에 양념이나 식재료가 묻은 상태로 태블릿/노트북을 터치하면 화면이 오염되고 기기가 손상되며 교차 오염 위험 발생.

---

## 🚀 2. 주요 기능 (Key Features)

### 1️⃣ 실시간 연기(Smoke) 감지 및 온디바이스 경보
* **Object Detection**: 파인튜닝된 YOLOv8 모델이 프레임 내 연기 발생을 실시간으로 추론.
* **즉각적 비상 알림**: 연기 감지 시 화면 레이아웃이 위험(RED) 상태로 변경되며 PC 스피커를 통해 온디바이스 알람(Beep) 출력.

### 2️⃣ Hands-Free 비접촉 제스처 제어 (No-Touch UX)
* **Pose Estimation**: `YOLOv8-Pose`를 통해 사람의 상체 관절(Keypoints) 좌표를 실시간 추적.
* **OS 이벤트 매핑**: 손 올리기 제스처 감지 시 `PyAutoGUI`를 이용해 키보드/마우스 명령어(Space, Arrow Keys 등)를 자동 전송하여 레시피 재생/일시정지 및 스크롤 제어.

---

## 🛡️ 3. 온디바이스 SW 및 시장 타당성 (Value Proposition)

* **초저지연성 (Low Latency)**: 네트워크 대기시간(Latency) 없이 0.03초 만에 화재 연기를 즉시 감지하여 현장 경보 울림.
* **프라이버시 완벽 보호 (Privacy Guard)**: 주방 내부 카메라 영상을 외부 클라우드로 송신하지 않고 단말 내부에서 처리 후 즉시 파기.
* **비용 절감 & 비연결성 (Cost Efficiency)**: 24시간 클라우드 서버 호출 비용 0원, 인터넷 미연결(Offline) 주방 환경에서도 완벽 작동.
* **확장성**: 스마트 가전(스마트 냉장고, 월패드, 인덕션 등) 디스플레이의 기본 탑재 SW로 확장 가능.

---

## 🏗️ 4. 프로젝트 구조 (Directory Structure)

본 프로젝트는 **단일 책임 원칙(SRP)**과 **객체지향 프로그래밍(OOP)** 규칙에 맞추어 모듈화되어 있습니다.

```text
smart_kitchen_assistant/
│
├── data/                      # 데이터 관리 폴더
│   └── raw_smoke_dataset/     # Roboflow에서 다운받은 연기 데이터셋
│
├── models/                    # AI 모델 가중치 (.pt) 파일 관리
│   ├── yolov8n-pose.pt        # 포즈 추정 경량 기본 모델
│   └── custom_smoke_best.pt   # 파인튜닝 완료된 연기 감지 모델
│
├── src/                       # 핵심 소스코드 (OOP 모듈)
│   ├── __init__.py
│   ├── smoke_detector.py      # [클래스] 연기 감지 전담 모듈
│   ├── gesture_controller.py  # [클래스] 손동작 추적 및 OS 조작 전담 모듈
│   └── kitchen_ui_overlay.py  # [클래스] 대시보드 UI 및 경고 시각화 전담 모듈
│
├── train_smoke_model.py       # [실행] 연기 감지 모델 학습 스크립트
├── main.py                    # [실행] 메인 애플리케이션 엔트리 포인트
└── requirements.txt           # 의존성 라이브러리 목록
